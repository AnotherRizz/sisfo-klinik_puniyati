...
@section('content')
    <h1 class="text-xl font-semibold mb-4">Data Pemeriksaan Kesehatan ANAK</h1>
    ...
    <div x-data="{ search: '' }" class="bg-white rounded shadow p-2 overflow-x-auto">
        ...
        <div class="w-full flex justify-between">
            ...
            <div class="mb-4 basis-1/2">
                <x-search-input :action="route('kia-anak.index')" name="search" placeholder="Cari nama, alamat, tanggal lahir, nomor rm" />
            </div>
            <x-paginate :options="[2, 5, 10, 15, 20]" :default="10" :action="route('kia-anak.index')" />
                    ...
                    <tr>
                       <td class="px-4 py-2 text-sm text-gray-900">{{ $loop->iteration }}</td>
                        ...
                        <td class="px-4 py-2 text-sm text-gray-900 flex gap-1">
                            @if ($item instanceof \App\Models\Pendaftaran)
                                <!-- Tombol Periksa -->
                                <a href="{{ route('kia-anak.create', ['pendaftaran_id' => $item->id]) }}"
                                    class="px-3 py-1 text-white bg-green-500 rounded text-xs hover:bg-green-600">
                                    Periksa
                                </a>
                            @else
                                <!-- Tombol Resume dan Detail -->
                                <a href="{{ route('kia-anak.resume', $item->id) }}" target="_blank"
                                    class="px-3 py-1 text-white bg-teal-500 rounded text-xs hover:bg-teal-600">Resume</a>
                                <a href="{{ route('kia-anak.edit', $item->id) }}"
