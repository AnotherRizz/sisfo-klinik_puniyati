                ...
                <table class="min-w-full text-xs text-left border border-gray-300 whitespace-normal break-words">
                    <thead class="bg-gray-100 text-gray-700">
                        <tr>
                            ...
                            <th class="border px-2 py-1 text-center">INFEKSI KOMPLIKASI </th>
                            <th class="border px-2 py-1 text-center">LOCHEA </th>
                            <th class="border px-2 py-1 text-center">FREK KUNJ </th>
                            <th class="border px-2 py-1 text-center">TGL LAHIR BAYI </th>
                            <th class="border px-2 py-1 text-center">JNS PERSALINAN </th>
                            <th class="border px-2 py-1 text-center">LUKA JAHIT </th>
                    ...
                    <tbody>
                        @foreach ($pemeriksaans as $index => $item)
                            ...
                            <tr class="border-b">
                                ...
                                <td class="border px-2 py-1">{{ $item->infeksi_kompli ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->lochea ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->frek_kunjungan ?? '-' }}</td>
                              <td class="border px-2 py-1">  {{ \Carbon\Carbon::parse($item->tgllahir)->translatedFormat('d/m/Y') ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->jnspersalinan ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->lukajahit ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->diagnosa ?? '-' }}</td>
                                <td class="border px-2 py-1">{{ $item->intervensi ?? '-' }}</td>
